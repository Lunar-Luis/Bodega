...
import { Producto, Config, Venta } from '../types';
    ...
    <div className="px-4 pt-4 pb-24 space-y-5">
      ...
      <div className="card">
        ...
        <input
          ...
          accept="image/*"
          className="hidden"
          onChange={handleCambiarFotoEdicion}
        />
        ...
        <div className="space-y-2 max-h-[30rem] overflow-y-auto">
          {productos.map((p) => (
            <div
              key={p.id}
              className={'flex items-center gap-3 p-3 rounded-2xl border transition-all ' + (p.activo ? 'bg-slate-50 border-slate-100 shadow-sm' : 'bg-slate-100/50 border-slate-200/50 opacity-60')}
            >
              <button
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs font-bold text-primary/70">
                    {formatearUSD(p.precioUSD)} | {formatearBs(p.precioBs ?? 0)}
                  </p>
                </div>
              </button>
              <button
      ...
      <div className="text-center">
        <button onClick={descargarRespaldo} className="text-xs text-slate-400 py-2 active:text-slate-600">
